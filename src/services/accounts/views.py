...
from accounts.db import User, OAuthClient, OAuthCode, Token, db
...
def oauth():
    ...
    return redirect(client.redirect_uri, params={"code": code}, code=302)
...
def provide_auth():
    ...
    oauth_client = OAuthClient.query.filter_by(id=credentials.client_id, secret=credentials.secret).first()
    if not oauth_client:
    ...
    token = uuid.uuid4()
    token_instance = Token(token=token, user_uid=code_instance.user_uid)
